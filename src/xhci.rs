...
impl PciXhciDriver {
    ...
    fn setup_xhc_registers(
        ...
        let op_regs =
            unsafe { Mmio::from_raw(bar0.addr().add(cap_regs.as_ref().caplength()) 
    ...
    async fn address_device(
        ...
        // 4.3.3 Device Slot Initalization
        ...
        // 4. Initalize the Transfer Ring for the Default Control Endpoint
        // 5. Initalize the input default control Endpoint 0 Context (6.2.3)
    ...
    async fn request_string_descriptor(
        ...
        .await?;
        Ok(String::from_utf16_lossy(&buf[2..])
            .to_string()
            .replace('\0', ""))
    }
...
struct CapabilityRegisters {
    ...
    hcsparams4: Volatile<u32>,
    dboff: Volatile<u32>,
    rtsoff: Volatile<u32>,
    hccpaams2: Volatile<u32>,
}
...
// [xhci_1_2] p.31
// The Device Context Base Address Arrat contains 256 Entries
// and supports up to 255 USB devices or hubs
// [xhci_1_2] p.59
// th first entry (SlotID = '0') in the Device Context Base
// Address Array is utilized by th xHCI Scratchpad mechanism.
#[repr(C, align(64))]
...
impl OperationalRegisters {
    ...
    fn set_cmd_ring_ctrl(&mut self, ring: &CommandRing) {
        self.crcr.write(
            ring.ring_phys_addr() | 1, /* Consumer Ring Cylce State */
        )
    }
    fn set_dcbaa_ptr(
        &mut self,
...
const _:() = assert!(size_of::<InterrputRegistersSet>() == 0x20);

#[repr(C)]
struct RuntimeRegisters {
    mfindex: Volatile<u32>,
    rsvdz: [u32; 7],
    irs: [InterrputRegistersSet; 1024],
}
...
impl EndpointContext {
    ...
    fn set_error_count(&mut self, error_count: u32) -> Result<()> {
        //info!("err_count = {error_count} {}", error_count & 0b11);
...
impl DeviceContext {
    ...
    fn set_last_valid_dci(&mut self, dci: usize) -> Result<()> {
        ...
        // requires the Context Entries field to be initalized to
        // defined by the target cofiguration
        if dci <= 31 {
...
impl Controller {
    ...
    pub fn notify_eq(&self, slot: u8, dci: usize) -> Result<()> {
        let db = self
            .regs
            .doorbell_regs
            .get(slot as usize)
...
impl EventRing {
    ...
    fn pop(&mut self) -> Result<Option<GenericTrbEntry>> {
        ...
        unsafe {
            let erdp = self.erdp.expect("erdp is no set");
...
// Limiting th size of TrbRing to be equal or less than 4096
// to avoid crossing 64kib boundaries. See Table6-1 of xhci spec.
...
impl GenericTrbEntry {
    ...
    fn cmd_result_ok(&self) -> Result<()> {
        if self.trb_type() != TrbType::CommandCompletionEvent as u32 {
            ...
            info!(
                "Cinoketuib cide was not success. actual = {}",
                self.completion_code()
            );
            Err("Completion Code was not success.")
        } else {
            Ok(())
        }
    }
    pub fn transfer_result_ok(&self) -> Result<()> {
        if self.trb_type() != TrbType::TransferEvent as u32 {
            Err("Not a Transfer event.")
        } else if self.completion_code() != 1 && self.completion_code() != 13 {
            info!(
                "Transfer failed. Actual CompletionCod = {}",
                self.completion_code()
            );
...
impl EventWaitInfo {
    fn matchs(&self, trb: &GenericTrbEntry) -> bool {
        if let Some(trb_type) = self.cond.trb_type {
            if trb_type as u32 != trb.trb_type() {
                return false
            }
        }
...
impl PortSc {
    fn new(bar: &BarMem64, cap_regs: &CapabilityRegisters) -> Self {
        ...
        for port in 1..=num_ports {
            // SAFETY: This is safe sice the result of ptr calculation
...
impl PortScEntry {
    ...
    fn assert_bit(&self, pos: usize) {
        const PRESERVER_MASK: u32 = 0b0100_1111_0000_0001_1111_1111_1110_0001;
        let portsc = self.ptr.lock();
        let old = unsafe { read_volatile( *portsc) };
        unsafe { write_volatile(*portsc, (old & PRESERVER_MASK) | (1 << pos));}
    }
    fn pp(&self) -> bool {
        // PP - Port Power - RWS
        self.bit(9)
