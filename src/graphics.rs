...
pub fn draw_test_pattern<T: Bitmap>(buf: &mut T) {
    ...
    draw_str_fg(buf, left, h * colors.len() as i64, colors[2], "0123456789");
    draw_str_fg(buf, left, h*colors.len() as i64 + 16, colors[2], "ABCDEF");
}
...
pub trait Bitmap {
    ...
    /// # Safety
    /// 
    /// Returned pointer is valid as long as th given coordinates are valid
    /// which means that passing is_in_*_range tests.
    unsafe fn unchecked_pixel_at_mut(&mut self, x: i64, y: i64) -> *mut u32 {
        self.buf_mut().add(
            ((y * self.pixel_per_line() + x) * self.byte_per_pixel())
              as usize,
        ) as *mut u32
    }
    ...
    fn pixel_at_mut(&mut self, x: i64, y: i64) -> Option<&mut u32> {
        if self.is_in_x_range(x) && self.is_in_y_range(y) {
            // SAFTY: (x, y) is always validated by the checks above.
...
fn calc_slopepoint(da:i64, db:i64, ia:i64) -> Option<i64> {
    if da < db {
        None
    } else if da == 0 {
        Some(0)
...
fn draw_line<T: Bitmap> (
    ...
    if dx >= dy {
        for (rx, ry) in (0..dx)
            .flat_map(|rx| calc_slopepoint(dx, dy, rx).map(|ry| (rx, ry))) 
        {
            draw_point(buf, color, x0 + rx * sx, y0 + ry*sy)?;
        }
    } else {
        for(rx, ry) in (0..dy)
            .flat_map(|ry| calc_slopepoint(dy, dx, ry).map(|rx| (rx, ry)))
        {
            draw_point(buf, color, x0 + rx * sx, y0 + ry*sy)?;
        }
    }
