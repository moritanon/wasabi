...
impl <const LEVEL:usize,NEXT: core::fmt::Debug>
    Table<LEVEL, NEXT>
{
    fn format(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
pub unsafe fn write_cs(cs: u16) {
    ...
    asm!(
        ...
        "push cx", // Construct a far pointer onthe stack
        "push rax", 
        "ljmp [rsp]",
              "2:",
...
struct InterruptInfo {
    ...
    // Should be aligned 16-byte boundaries to pass the
    // aliginment checks done by FXSAVE /FARSTOR
    fpu_context: FPUContenxt,
    _dummy: u64,
...
// In IA32e mode, the RSP is aligned to a 16-byte boundary
// before pushing the stack frame

/// This generateds interrupt_entrypointN()
...
macro_rules! interrupt_entrypoint {
    ($index:literal) => {
        global_asm!(concat!(
            ...
            "push 0    // No error code\n",
            "push rcx  // Save rcx first to resuse\n",
...
global_asm!(
    ...
    inthandler_common:
    // General purpos registers (except rsp and rcx)
    push r15
    push r14
    push r13
    push r12
    ...
    sub rsp, 512 + 8
    fxsave64[rsp]
    // 1st parameter: point to the saved CPU state
    mov rdi, rsp
...
extern "sysv64" fn int_handler_unimplemented() {
    panic!("unexepted interrupt!");
}

// PDDRTTTT (TTTT: type, R: reserved, D: DPL, P presend)
...
#[derive(Copy, Clone)]
enum IdtAttr {
    // Without _NotPresent value, MaybeUninit::zerod() on
    // this struct will be undefined behavior.
    _NotPresent = 0,
    IntGateDPL0 = BIT_FLAGS_INGATE | BIT_FLAGS_PRESENT | BIT_FLAGS_DPL0,
    IntGateDPL3 = BIT_FLAGS_INGATE | BIT_FLAGS_PRESENT | BIT_FLAGS_DPL3,
}
