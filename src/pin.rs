...
/// # Safety
/// Implementing this trait is safe only when target type can be contructed
...
/// since the bytes can note be interpreted as the original type.
pub unsafe trait IntoPinnedMutableSlice: Sized + Copy + Clone {
    ...
    fn as_mut_slice_sized(
        self: Pin<&mut Self>,
        size: usize,
    ) -> Result<Pin<&mut [u8]>> {
        if size > size_of::<Self>() {
            Err("Cannot take mut slice longer than this object.")
        } else {
            Ok(Pin::new(unsafe {
                slice::from_raw_parts_mut(
                    self.get_unchecked_mut() as *mut Self as *mut u8,
